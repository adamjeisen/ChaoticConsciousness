...
def compute_eigs(pts, dt, window=1, stride=None, return_sigma_norms=False, PCA_dim=-1):
    # -------------------
    ...
    for i in tqdm(range(num_win)):
        ...
        results['eigs'][i] = np.abs(np.real(e))
...
# dt, window, stride are in seconds
# pts has shape (num_time_steps, num_dims)
def get_data_sigma_norms(pts, dt, window=1, stride=None):
    n = pts.shape[0]
    k = int(window/dt) # window size in time steps
    # default to window stride
    if stride is None:
